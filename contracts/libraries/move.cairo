...
namespace move_strategy:
    ...
    func move_all_ships{syscall_ptr : felt*, range_check_ptr, grid : Grid}(ship_addresses : felt*):
        alloc_locals
        let (grid_iterator) = grid_access.start()
        # Keeping the id of the last ship that moved so we can ignore it when we iterate on the grid
        local last_ship_moved = -1
        let (vector_array : Vector2*) = alloc()
        let vector_array_len = 0
        with grid_iterator, vector_array, vector_array_len:
    ...
    namespace internal:
        func move_dust_loop{
            ...
            # %{ print(f"grid iterator x,y {ids.grid_iterator.x}, {ids.grid_iterator.y}") %}
            let (done) = grid_access.done()
            if done == 1:
                %{ print(f"grid iterator move dust done  x,y {ids.grid_iterator.x}, {ids.grid_iterator.y}") %}
        ...
        func try_move_single_dust{
            ...
            # %{ print(f"grid iterator move dust  x,y {ids.grid_iterator.x}, {ids.grid_iterator.y}") %}
        ...
        func move_ship_loop{
            ...
            let (done) = grid_access.done()
            if done == 1:
                %{ print(f"grid iterator ship loop done x,y {ids.grid_iterator.x}, {ids.grid_iterator.y}") %}
        ...
        func try_move_single_ship{
            ...
            assert [vector_array + vector_array_len * Vector2.SIZE] = grid_iterator
            let vector_array_len = vector_array_len + 1
            # %{ print(f"sip_id / last ship moved {ids.ship_id} / {ids.last_ship_moved}") %}

            # Not doing anything is the ship was already moved
            # if ship_id == last_ship_moved:
            #     return ()
            # end

            # move_single_ship(ship_id, ship_addresses[ship_id - 1], cell)
            # let last_ship_moved = ship_id
        ...
        func move_single_ship{syscall_ptr : felt*, range_check_ptr, grid : Grid}(
            ...
            # Call ship interface
            %{ print(f"move ship : ship_id {ids.ship_id}, contract {ids.ship_contract}") %}
            ...
            %{ print(f"next cell at position; ship id : {ids.cell_new.ship_id}") %}
        ...
        func ship_can_collide{range_check_ptr, grid : Grid}(new_position : Vector2) -> (
            ...
            %{ print(f"new pos : {ids.new_position.x}, {ids.new_position.y}") %}
            let (local has_ship_in_current_grid) = cell_access.has_ship{cell=destination}()
            %{ print(f"has ship in currentgrid: {ids.has_ship_in_current_grid}") %}
